...
export function RegExpBuiltinExec(R, S) {
  ...
  while (matchSucceeded === false) {
    ...
    if (lastIndex > length) {
      ...
      if (global || sticky) {
        ...
        Q(Set(R, new Value('lastIndex'), 𝔽(0), Value.true));
      }
      // ii. Return null.
      return Value.null;
    }
  ...
  if (global || sticky) {
    ...
    Q(Set(R, new Value('lastIndex'), 𝔽(e), Value.true));
  }
  ...
  X(CreateDataPropertyOrThrow(A, new Value('index'), 𝔽(lastIndex)));
...
function RegExpProto_match([string = Value.undefined], { thisValue }) {
  ...
  if (global === Value.false) {
    ...
    Q(Set(rx, new Value('lastIndex'), 𝔽(0), Value.true));
    ...
    while (true) {
      ...
      if (result === Value.null) {
        ...
        if (matchStr.stringValue() === '') {
          ...
          Q(Set(rx, new Value('lastIndex'), 𝔽(nextIndex), Value.true));
        }
        // 4. Set n to n + 1.
        n += 1;
      }
...
function RegExpProto_replace([string = Value.undefined, replaceValue = Value.undefined], { thisValue }) {
  ...
  for (const result of results) {
    ...
    if (functionalReplace === Value.true) {
      ...
      replacerArgs.push(𝔽(position), S);
...
function RegExpProto_split([string = Value.undefined, limit = Value.undefined], { thisValue }) {
  ...
  let q = p;
  while (q < size) {
    Q(Set(splitter, new Value('lastIndex'), 𝔽(q), Value.true));
    const z = Q(RegExpExec(splitter, S));
    if (z === Value.null) {
