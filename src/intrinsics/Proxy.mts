...
function Proxy_revocable([target = Value.undefined, handler = Value.undefined]) {
  ...
  const revoker = X(CreateBuiltinFunction(steps, length, Value.of(''), ['RevocableProxy']));
  ...
  X(CreateDataProperty(result, Value.of('proxy'), p));
  // 8. Perform ! CreateDataPropertyOrThrow(result, "revoke", revoker).
  X(CreateDataProperty(result, Value.of('revoke'), revoker));
  // 9. Return result.
  return result;
}
