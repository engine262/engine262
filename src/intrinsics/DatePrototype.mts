...
function DateProto_toJSON(args, { thisValue }) {
  ...
  return Q(Invoke(O, Value.of('toISOString')));
}
...
function TimeString(tv) {
  ...
  return Value.of(`${hour}:${minute}:${second} GMT`);
}
...
function DateString(tv) {
  ...
  const yearSign = yv >= 0 ? '' : '-';
  const year = Value.of(String(Math.abs(yv)));
  const paddedYear = X(StringPad(year, F(4), Value.of('0'), 'start')).stringValue();
  return Value.of(`${weekday} ${month} ${day} ${yearSign}${paddedYear}`);
}
...
export function TimeZoneString(tv) {
  ...
  const tzName = '';
  return Value.of(`${offsetSign}${offsetHour}${offsetMin}${tzName}`);
}
...
export function ToDateString(tv) {
  ...
  const t = LocalTime(tv);
  return Value.of(`${DateString(t).stringValue()} ${TimeString(t).stringValue()}${TimeZoneString(t).stringValue()}`);
}
...
function DateProto_toTimeString(args, { thisValue }) {
  ...
  const t = LocalTime(tv);
  return Value.of(`${TimeString(t).stringValue()}${TimeZoneString(tv).stringValue()}`);
}
...
function DateProto_toUTCString(args, { thisValue }) {
  ...
  const year = Value.of(String(Math.abs(yv)));
  const paddedYear = X(StringPad(year, F(4), Value.of('0'), 'start')).stringValue();
  return Value.of(`${weekday}, ${day} ${month} ${yearSign}${paddedYear} ${TimeString(tv).stringValue()}`);
}
