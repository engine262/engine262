...
function DateProto_getTimezoneOffset(args, { thisValue }) {
  ...
  return 𝔽((t.numberValue() - LocalTime(t).numberValue()) / msPerMinute);
}
...
function DateString(tv) {
  ...
  const paddedYear = X(StringPad(year, 𝔽(4), new Value('0'), 'start')).stringValue();
...
export function TimeZoneString(tv) {
  ...
  const offsetMin = String(MinFromTime(𝔽(Math.abs(offset))).numberValue()).padStart(2, '0');
  const offsetHour = String(HourFromTime(𝔽(Math.abs(offset))).numberValue()).padStart(2, '0');
  const tzName = '';
