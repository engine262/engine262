...
function* IteratorHelperPrototype_return(_args: Arguments, { thisValue }: FunctionCallContext): ValueEvaluator {
  ...
  if (O.GeneratorState === 'suspended-start') {
    ...
    Q(IteratorClose((O as unknown as { UnderlyingIterator: IteratorRecord }).UnderlyingIterator, NormalCompletion(undefined)));
...
export function bootstrapIteratorHelperPrototype(realmRec: Realm) {
  ...
  ], realmRec.Intrinsics['%IteratorPrototype%'], 'Iterator Helper');
