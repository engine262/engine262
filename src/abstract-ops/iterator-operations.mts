...
export function GetIterator(obj, hint, method) {
  ...
  const nextMethod = Q(GetV(iterator, Value.of('next')));
...
export function IteratorComplete(iterResult) {
  ...
  return EnsureCompletion(ToBoolean(Q(Get(iterResult, Value.of('done')))));
}

/** http://tc39.es/ecma262/#sec-iteratorvalue */
export function IteratorValue(iterResult) {
  Assert(iterResult instanceof ObjectValue);
  return EnsureCompletion(Q(Get(iterResult, Value.of('value'))));
}
...
export function IteratorClose(iteratorRecord, completion) {
  ...
  let innerResult = EnsureCompletion(GetMethod(iterator, Value.of('return')));
...
export function CreateIterResultObject(value, done) {
  ...
  X(CreateDataProperty(obj, Value.of('value'), value));
  X(CreateDataProperty(obj, Value.of('done'), done));
  return obj;
}
...
export function CreateAsyncFromSyncIterator(syncIteratorRecord) {
  ...
  const nextMethod = X(Get(asyncIterator, Value.of('next')));
  return {
...
export function AsyncFromSyncIteratorContinuation(result, promiseCapability) {
  ...
  const onFulfilled = X(CreateBuiltinFunction(unwrap, 1, Value.of(''), ['Done']));
