...
function ExecuteAsyncModule(module) {
  ...
  const onFulfilled = CreateBuiltinFunction(fulfilledClosure, 0, Value.of(''), ['Module']);
  ...
  const onRejected = CreateBuiltinFunction(rejectedClosure, 0, Value.of(''), ['Module']);
...
export function CreateDefaultExportSyntheticModule(defaultExport, realm, hostDefined) {
  ...
  const closure = (module) => { // eslint-disable-line arrow-body-style
    ...
    return Q(module.SetSyntheticExport(Value.of('default'), defaultExport));
  };
  // 2. Return CreateSyntheticModule(« "default" », closure, realm)
  return CreateSyntheticModule([Value.of('default')], closure, realm, hostDefined);
}
