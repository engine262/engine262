...
export function PerformEval(x, callerRealm, strictCaller, direct) {
  ...
  // 4. Let evalRealm be the current Realm Record.
  ...
  if (thisEnvRec instanceof FunctionEnvironmentRecord) {
    ...
    // d. If F.[[ConstructorKind]] is derived, let inDerivedConstructor be true; otherwise, let inDerivedConstructor be false.
    ...
    inFunction = false;
    // b. Let inFunction be false.
    inMethod = false;
    // c. Let inDerivedConstructor be false.
    inDerivedConstructor = false;
  }
  // 8. Perform the following substeps in an implementation-dependent order, possibly interleaving parsing and error detection:
  ...
  // 11. Let runningContext be the running execution context.
  const runningContext = surroundingAgent.runningExecutionContext;
  let lexEnv;
  let varEnv;
  // 12. NOTE: If direct is true, runningContext will be the execution context that performed the direct eval.
  ...
  // 16. If runningContext is not already suspended, suspend runningContext.
  // 17. Let evalContext be a new ECMAScript code execution context.
  const evalContext = new ExecutionContext();
  // 18. Set evalContext's Function to null.
  evalContext.Function = Value.null;
  // 19. Set evalContext's Realm to evalRealm.
  evalContext.Realm = evalRealm;
  // 20. Set evalContext's ScriptOrModule to runningContext's ScriptOrModule.
  ...
  // 23. Push evalContext onto the execution context stack.
  ...
  // 27. Suspend evalContext and remove it from the execution context stack.
