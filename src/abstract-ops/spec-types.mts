...
export function FromPropertyDescriptor(Desc) {
  ...
  if (Desc.Value !== undefined) {
    X(CreateDataProperty(obj, Value.of('value'), Desc.Value));
  }
  if (Desc.Writable !== undefined) {
    X(CreateDataProperty(obj, Value.of('writable'), Desc.Writable));
  }
  if (Desc.Get !== undefined) {
    X(CreateDataProperty(obj, Value.of('get'), Desc.Get));
  }
  if (Desc.Set !== undefined) {
    X(CreateDataProperty(obj, Value.of('set'), Desc.Set));
  }
  if (Desc.Enumerable !== undefined) {
    X(CreateDataProperty(obj, Value.of('enumerable'), Desc.Enumerable));
  }
  if (Desc.Configurable !== undefined) {
    X(CreateDataProperty(obj, Value.of('configurable'), Desc.Configurable));
  }
...
export function ToPropertyDescriptor(Obj) {
  ...
  const hasEnumerable = Q(HasProperty(Obj, Value.of('enumerable')));
  if (hasEnumerable === Value.true) {
    const enumerable = ToBoolean(Q(Get(Obj, Value.of('enumerable'))));
    desc.Enumerable = enumerable;
  }
  const hasConfigurable = Q(HasProperty(Obj, Value.of('configurable')));
  if (hasConfigurable === Value.true) {
    const conf = ToBoolean(Q(Get(Obj, Value.of('configurable'))));
    desc.Configurable = conf;
  }
  const hasValue = Q(HasProperty(Obj, Value.of('value')));
  if (hasValue === Value.true) {
    const value = Q(Get(Obj, Value.of('value')));
    desc.Value = value;
  }
  const hasWritable = Q(HasProperty(Obj, Value.of('writable')));
  if (hasWritable === Value.true) {
    const writable = ToBoolean(Q(Get(Obj, Value.of('writable'))));
    desc.Writable = writable;
  }
  const hasGet = Q(HasProperty(Obj, Value.of('get')));
  if (hasGet === Value.true) {
    const getter = Q(Get(Obj, Value.of('get')));
  ...
  const hasSet = Q(HasProperty(Obj, Value.of('set')));
  if (hasSet === Value.true) {
    const setter = Q(Get(Obj, Value.of('set')));
