  ...
  * OwnPropertyKeys(): ObjectSlotReturn['OwnPropertyKeys'] {
  ...
  mark(trace: GCTrace) {
    for (const [key, descriptor] of this.properties) {
      ...
      trace.strong(`get ${name}`, descriptor.Get, referencePropertyReason(key));
      trace.strong(`set ${name}`, descriptor.Set, referencePropertyReason(key));
    }
    ...
    this.PrivateElements.forEach((element, index) => {
      ...
      trace.strong(`PrivateElements[${index}].getter`, element.Get, 'private-element');
      trace.strong(`PrivateElements[${index}].setter`, element.Set, 'private-element');
    });
...
export @callable() class Descriptor {
  ...
  mark(trace: GCTrace) {
    ...
    trace.strong('Get', this.Get, 'internal-slot');
    trace.strong('Set', this.Set, 'internal-slot');
  }
}

function referencePropertyName(key: PropertyKeyValue): string {
  if (key instanceof JSStringValue) return key.stringValue();
  return typeof key.Description === 'string'
