...
function nodeName(value: object, agent: Agent): string {
  ...
  if (value instanceof PrivateName) return `PrivateName(${value.Description.stringValue()})`;
  if (value instanceof ObjectValue) {
    if ('Call' in value) return 'Function';
    const typedArrayName = internalSlot(value, 'TypedArrayName');
    if (typedArrayName instanceof JSStringValue) return typedArrayName.stringValue();
    ...
    if (prototype instanceof ObjectValue) {
      ...
      if (constructor instanceof ObjectValue) {
        ...
        if (initialName instanceof JSStringValue && initialName.stringValue()) {
          return initialName.stringValue();
        }
      }
    }
    return 'Object';
