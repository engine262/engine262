// @ts-nocheck
import { ExecutionContext, GCMarker, surroundingAgent } from './engine.mjs';
import {
  Value, Descriptor, JSStringValue, NumberValue, ObjectValue, UndefinedValue, NullValue,
} from './value.mjs';
import {
  ToString, DefinePropertyOrThrow, CreateBuiltinFunction, FunctionObjectValue,
  ...
  * [Symbol.iterator]() {
    for (const [key, value] of this.map.entries()) {
      if (typeof key === 'string' || typeof key === 'number') {
        yield [Value.of(key), value] as const;
      } else {
...
export function handleInResume<T extends((...arg: any[]) => any)>(fn: T, ...args: Parameters<T>) {
  const bound = () => fn(...args);
...
export class CallSite {
  ...
  inheritedLastCallNode: ParseNode | null = null;
  constructCall = false;
  ...
  getFunctionName() {
    if (!(this.context.Function instanceof NullValue)) {
      const name = this.context.Function.properties.get(Value.of('name'));
      if (name) {
  ...
  setCallLocation(node: ParseNode) {
  ...
  X(DefinePropertyOrThrow(O, name, Descriptor({
    Get: CreateBuiltinFunction(() => {
      if (cache === null) {
        ...
        cache = Value.of(errorString);
      }
      return cache;
    }, 0, name, [], undefined, undefined, Value.of('get')),
    Set: CreateBuiltinFunction(([value = Value.undefined]) => {
      cache = value;
      return Value.undefined;
    }, 1, name, [], undefined, undefined, Value.of('set')),
    Enumerable: Value.false,
    Configurable: Value.true,
  })));
}

export function callable(onCalled = (target: any, _thisArg: any, args: any) => Reflect.construct(target, args)): ClassDecorator {
  return function decoartor(classValue) {
...
export const isArray: (arg: any) => arg is readonly any[] = Array.isArray;
